...
#[rtic::app(device = stm32f4xx_hal::pac, peripherals = true, dispatchers = [USART1, USART2])]
mod app {
    use crate::{
        // init::Init,
    ...
    use stm32f4::stm32f411::{ADC1, DMA2};
    use stm32f4xx_hal::{
        adc::{
    ...
    fn init(cx: init::Context) -> (Shared, Local) {
        ...
        let miso = NoMiso::new();
        let rst = gpiob.pb0.into_push_pull_output().speed(Speed::Low);
        let cs = gpiob.pb2.into_push_pull_output().speed(Speed::Low);
        ...
        // SPI interface
        let spi = Spi::new(dp.SPI1, (sck, miso, mosi), mode, 1.MHz(), &clocks);
    ...
    #[task(local = [display], shared = [menu, data, settings, item_setting, is_clear], priority = 1)]
    async fn display_menu_task(cx: display_menu_task::Context) {
        ...
        loop {
            (
                ...
                &mut shared.data,
                &mut shared.settings,
                &mut shared.item_setting,
                &mut shared.is_clear,
            )
                .lock(|menu, data, settings, item_setting, is_clear| {
                    if *is_clear {
                        display.clear(BACKGROUND_COLOR).unwrap();
                        *is_clear = false;
                    }
                    ...
                    match menu {
                        Menu::Main => {
                            ui::main::draw_grid_main(display);
                            ui::main::draw_labels_main(display);
                            ui::main::draw_main(display, data);
                        }
                        Menu::Fan(fan) => {
                            ui::setting::draw_grid_settings(display);
                            ui::setting::draw_labels_settings(display, *fan);
                            ui::setting::draw_settings(
                                display,
                                &settings.fans[*fan - 1],
                                item_setting,
                            )
            ...
            cx.shared.buttons.lock(|buttons| {
                match buttons {
                    ...
                    AllButton::Minus(minus) => {
                        match minus {
                            ButtonState::ShortPress => info!("MinusShortPress"),
                            ButtonState::LongPress => info!("MinusLongPress"),
                            ButtonState::LongPressDuration(_) => info!("MinusLongPressDuration"),
                        }
                    }
                    AllButton::Ok(ok) => {
                        match ok {
                            ButtonState::ShortPress => info!("OkShortPress"),
                            ButtonState::LongPress => info!("OkLongPress"),
                            ButtonState::LongPressDuration(_) => info!("OkLongPressDuration"),
                        }
                    }
                    AllButton::Plus(plus) => {
                        match plus {
                            ButtonState::ShortPress => info!("PlusShortPress"),
                            ButtonState::LongPress => info!("PlusLongPress"),
                            ButtonState::LongPressDuration(_) => info!("PlusLongPressDuration"),
                        }
                    }
                }
                *buttons = AllButton::No;
    ...
    async fn button_task(mut cx: button_task::Context) {
        ...
        let button_config = ButtonConfig::new(
            MyDuration::millis(30),
            MyDuration::millis(1),
            MyDuration::millis(700),
            ButtonMode::PullUp,
            100,
        );
    ...
    #[task(binds = TIM1_TRG_COM_TIM11, local = [counter_hz], shared = [frequency, interrupt_count], priority = 3)]
    ...
    #[task(binds = DMA2_STREAM0, shared = [transfer], local = [buffer], priority = 2)]
